...
func main() {
	...
	mux.Handle("/api/profile", authz.RequireAuth(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		if prof.PvPRank == "" { prof.PvPRank = "Knight" }
...
func readProfile(name string) protocol.Profile {
        ...
        if err != nil {
                // defaults for new users
                if p.PvPRating == 0 { p.PvPRating = 1200 }
                if p.PvPRank == "" { p.PvPRank = "Knight" }
                if p.Avatar == "" { p.Avatar = "default.png" }
                return p
        }
        ...
        if p.PvPRank == "" { p.PvPRank = "Knight" }
        if p.Avatar == "" { p.Avatar = "default.png" }
        if p.Armies == nil { p.Armies = map[string][]string{} }
        if p.UnitXP == nil { p.UnitXP = map[string]int{} }
        if p.Resources == nil { p.Resources = map[string]int{} }
        return p
}
