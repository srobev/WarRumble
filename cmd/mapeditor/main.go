...
func (e *editor) Update() error {
	...
	for _, k := range inpututil.AppendJustPressedKeys(nil) {
		if k == ebiten.KeyS && (ebiten.IsKeyPressed(ebiten.KeyControl) || ebiten.IsKeyPressed(ebiten.KeyMeta)) {
			e.save()
		}
		if k == ebiten.KeyG && !e.bgFocus && !e.nameFocus {
		...
		if wy != 0 {
			if mx >= panelX && mx < panelX+panelW && my >= panelY && my < panelY+panelH {
				...
				if len(e.assets) > maxRows {
					...
					if e.assetsScroll > maxStart {
						e.assetsScroll = maxStart
					}
				}
			}
		}
...
func (e *editor) Draw(screen *ebiten.Image) {
	...
	for i, lb := range labels {
		...
		if e.tool == i {
			col = color.NRGBA{0x55, 0x77, 0xaa, 0xff}
		}
	...
	if e.status != "" {
		text.Draw(screen, e.status, basicfont.Face7x13, saveX+110, saveY+16, color.NRGBA{180, 220, 180, 255})
	}
	...
	if inpututil.IsMouseButtonJustPressed(ebiten.MouseButtonLeft) {
		...
		if mx >= nameX && mx < nameX+220 && my >= nameY && my < nameY+24 {
			e.nameFocus = true
			e.bgFocus = false
		} else if !(mx >= bx && mx < bx+260 && my >= by && my < by+24) {
			if !(mx >= lx && mx < lx+lw && my >= ly && my < ly+lh) && !(mx >= sx && mx < sx+sw && my >= sy && my < sy+sh) && !(mx >= cx && mx < cx+cw && my >= cy && my < cy+ch) && !(mx >= ax && mx < ax+aw && my >= ay && my < ay+ah) {
				e.nameFocus = false
			}
		}
	...
	if e.assetsOpen {
		...
		for i := 0; i < maxRows && start+i < len(e.assets); i++ {
			...
			show := p
			if len(show) > 32 {
				show = "…" + show[len(show)-31:]
			}
			var acol color.Color = color.White
			if e.assetsSel == start+i {
