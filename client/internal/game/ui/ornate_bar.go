...
func LoadOrnateBar() *OrnateBar {
    // Try multiple filenames to match your asset names
    tryLoad := func(paths ...string) *ebiten.Image {
        for _, p := range paths {
            if img, _, err := ebitenutil.NewImageFromFile(p); err == nil && img != nil {
                return img
            }
        }
        return nil
    ...
    frame := tryLoad(
        ...
        "assets/ui/bar_frame.png",
    )
    // level badge/circle
    badge := tryLoad(
        "assets/ui/level_bar.png",
        "assets/ui/level_badge.png",
    )
    ob := &OrnateBar{
        Badge:    badge,
        Frame:    frame,
    ...
    if b, err := os.ReadFile("assets/ui/health_bar.meta.json"); err == nil {
        if json.Unmarshal(b, &m) == nil {
            if m.Mode != "" { ob.Mode = m.Mode }
            if m.PadX != 0 || m.PadY != 0 { ob.PadX, ob.PadY = m.PadX, m.PadY }
            if m.WellOfs[0] != 0 || m.WellOfs[1] != 0 { ob.WellOfs = image.Pt(m.WellOfs[0], m.WellOfs[1]) }
            if m.WellSize[0] != 0 || m.WellSize[1] != 0 { ob.WellSize = image.Pt(m.WellSize[0], m.WellSize[1]) }
            if m.BadgeScale > 0 { ob.BadgeScale = m.BadgeScale }
        }
    }
    return ob
}
